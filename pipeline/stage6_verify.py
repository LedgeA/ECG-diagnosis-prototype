"""Stage 6 - check the corpus before anything is trained on it.
...
def load_index() -> list[dict]:
    if not C.INDEX_CSV.exists():
        sys.exit("index.csv missing - run stage5 first")
...
def check_integrity(rows: list[dict]) -> bool:
    ...
    expected = {r["record"]: int(C.RENDERS_PER_RECORD[r["cls"]]) for r in rows}
    actual = Counter(r["record"] for r in rows)
    wrong_n = {t: (actual[t], n) for t, n in expected.items() if actual[t] != n}

    for label, bad in (("patients spanning splits", bad_patients),
                       ("records in two classes", bad_records),
                       ("missing image files", missing),
                       ("wrong render count", wrong_n)):
...
def check_balance(rows: list[dict]) -> bool:
    ...
    by = Counter((r["cls"], r["split"]) for r in rows)
    print(f"    {'class':9s}{'train':>8s}{'val':>7s}{'test':>7s}{'total':>8s}")
    for cls in C.CLASSES:
        tr, va, te = (by[(cls, s)] for s in ("train", "val", "test"))
        print(f"    {cls:9s}{tr:8d}{va:7d}{te:7d}{tr+va+te:8d}")

    print(f"\n    {'class':9s}" + "".join(f"{s:>10s}" for s in ("stemi", "ptbxl", "chapman")))
    grid = Counter((r["cls"], r["source"]) for r in rows)
    for cls in C.CLASSES:
        print(f"    {cls:9s}" + "".join(f"{grid[(cls,s)]:10d}"
...
def check_leakage(rows: list[dict], per_source: int = 400, size: int = 64) -> bool:
    ...
    # Group by patient, or the probe just memorises records: the same record is
    # rendered 2-4 times, so an image-level split puts near-duplicates on both
    # sides and reports leakage that is really duplication.
